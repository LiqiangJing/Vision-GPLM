...
class MultiModal_BART(torch.nn.Module):
    def __init__(self, config):
        ...
        # self.concat_linear = torch.nn.Linear(768, 768)
        self.map = torch.nn.Linear(1024, 768)
        self.img_encoder = torch.load("./img_encoder.pkl")
        ...
        # self.img_encoder._modules['head'] = torch.nn.Linear(1024, 768)
        # self.img_norm = torch.nn.LayerNorm(768)

    def forward(self, input_ids, attention_mask, decoder_ids, decoder_attention_mask, img=None):
    ...
    def get_img_emb(self, img):
        ...
        # print(x.shape)
        return x
    def generate_text(self, input_ids, attention_mask, eval_beams=10, early_stopping=True, img=None):
        encoder_outputs = self.bart.model.encoder(input_ids=input_ids, attention_mask=attention_mask)
        last_hidden = encoder_outputs[0]
        if img is not None:
            img_emb = self.get_img_emb(img)
        ...
        generate_ids = self.bart.generate(
            input_ids=None, attention_mask=None, use_cache=True,
            decoder_start_token_id=tokenizer.pad_token_id, num_beams=10, max_length=self.config.max_length_tgt,
            early_stopping=early_stopping, encoder_outputs=encoder_outputs
        )

        return [tokenizer.decode(w, skip_special_tokens=True, clean_up_tokenization_spaces=True) for w in generate_ids]
